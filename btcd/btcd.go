...
var Cmd = &cobra.Command{
	Use:   "btcd",
	Short: "start btcd endpoint",
	Run: func(cmd *cobra.Command, args []string) {
		if err := Btcd(nil); err != nil {
			btcdLog.Error(err)
			os.Exit(1)
		}
...
func Btcd(serverChan chan<- *Server, opts ...Option) error {
	...
	if cfg.DropAddrIndex {
		if err := indexers.DropAddrIndex(db, signal.InterruptHandlersDone); err != nil {
			btcdLog.Errorf("%v", err)
			return err
		}
		return nil
	...
	if cfg.DropTxIndex {
		if err := indexers.DropTxIndex(db, signal.InterruptHandlersDone); err != nil {
			btcdLog.Errorf("%v", err)
			return err
		}
		return nil
	...
	if cfg.DropCfIndex {
		if err := indexers.DropCfIndex(db, signal.InterruptHandlersDone); err != nil {
			btcdLog.Errorf("%v", err)
			return err
		}
		return nil
	...
	server, err := newServer(cfg.Listeners, cfg.AgentBlacklist, cfg.AgentWhitelist, db, activeNetParams.Params, signal.InterruptHandlersDone)
	if err != nil {
