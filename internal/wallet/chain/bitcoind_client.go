...
type BitcoindClient struct {
	...
	notificationQueue *queue.ConcurrentQueue
...
var _ chain.Interface = (*BitcoindClient)(nil)
...
func (c *BitcoindClient) NotifySpent(outPoints []*wire.OutPoint) error {
	...
	for _, op := range outPoints {
		...
		if err != nil {
			log.Log.Errorf("Unable to get raw transaction for %v, "+
				"err: %v", txid, err)
			continue
		}
		...
		if err != nil {
			log.Log.Errorf("Cannot create transaction record for tx: "+
				"%v, err: %v", tx.Hash(), err)
			continue
		}
...
func (c *BitcoindClient) RescanBlocks(
	...
	for _, hash := range blockHashes {
		...
		if err != nil {
			log.Log.Warnf("Unable to get header %s from bitcoind: %s",
				hash, err)
			continue
		}
		...
		if err != nil {
			log.Log.Warnf("Unable to get block %s from bitcoind: %s",
				hash, err)
			continue
		}
...
func (c *BitcoindClient) Start() error {
	...
	c.notificationQueue.ChanIn() <- chain.ClientConnected{}
		...
		case update := <-c.rescanUpdate:
			...
			case chainhash.Hash:
				if err := c.rescan(update); err != nil {
					log.Log.Errorf("Unable to complete chain "+
						"rescan: %v", err)
				}
			default:
				log.Log.Warnf("Received unexpected filter type %T",
					update)
			}
		case <-c.quit:
			return
...
func (c *BitcoindClient) ntfnHandler() {
	...
	for {
		...
		case tx := <-c.txNtfns:
			...
			if err != nil {
				log.Log.Errorf("Unable to filter transaction %v: %v",
					txDetails.Hash(), err)
			}
		...
		case newBlock := <-c.blockNtfns:
			...
			if err := c.reorg(bestBlock, newBlock); err != nil {
				log.Log.Errorf("Unable to process chain reorg: %v",
					err)
			}
		case <-c.quit:
			return
...
func (c *BitcoindClient) onBlockConnected(hash *chainhash.Hash, height int32,
	...
	if c.shouldNotifyBlocks() {
		select {
		case c.notificationQueue.ChanIn() <- chain.BlockConnected{
			Block: wtxmgr.Block{
				Hash:   *hash,
				Height: height,
			},
...
func (c *BitcoindClient) onFilteredBlockConnected(height int32,
	...
	if c.shouldNotifyBlocks() {
		select {
		case c.notificationQueue.ChanIn() <- chain.FilteredBlockConnected{
...
func (c *BitcoindClient) onBlockDisconnected(hash *chainhash.Hash, height int32,
	...
	if c.shouldNotifyBlocks() {
		select {
		case c.notificationQueue.ChanIn() <- chain.BlockDisconnected{
			Block: wtxmgr.Block{
				Hash:   *hash,
				Height: height,
			},
...
func (c *BitcoindClient) onRelevantTx(tx *wtxmgr.TxRecord,
	...
	if err != nil {
		log.Log.Errorf("Unable to send onRelevantTx notification, failed "+
			"parse block: %v", err)
		return
	}

	select {
	case c.notificationQueue.ChanIn() <- chain.RelevantTx{
		TxRecord: tx,
		Block:    block,
	}:
	case <-c.quit:
...
func (c *BitcoindClient) onRescanProgress(hash *chainhash.Hash, height int32,
	...
	select {
	case c.notificationQueue.ChanIn() <- &chain.RescanProgress{
		Hash:   *hash,
		Height: height,
		Time:   timestamp,
	}:
...
func (c *BitcoindClient) onRescanFinished(hash *chainhash.Hash, height int32,
	...
	select {
	case c.notificationQueue.ChanIn() <- &chain.RescanFinished{
		Hash:   hash,
		Height: height,
		Time:   timestamp,
	}:
...
func (c *BitcoindClient) reorg(currentBlock waddrmgr.BlockStamp,
	...
	log.Log.Debugf("Possible reorg at block: height=%v, hash=%v", bestHeight,
		bestHash)

	if bestHeight < currentBlock.Height {
		log.Log.Debugf("Detected multiple reorgs: best_height=%v below "+
	...
	for previousBlock != currentHeader.PrevBlock {
		...
		log.Log.Debugf("Disconnecting block: height=%v, hash=%v",
...
func (c *BitcoindClient) FilterBlocks(
	...
	blockFilterer := chain.NewBlockFilterer(c.chainConn.cfg.ChainParams, req)
...
func (c *BitcoindClient) filterBlock(block *wire.MsgBlock, height int32,
	...
	if c.shouldNotifyBlocks() {
		log.Log.Debugf("Filtering block %d (%s) with %d transactions",
	...
	for i, tx := range block.Transactions {
		...
		if err != nil {
			log.Log.Warnf("Unable to filter transaction %v: %v",
				*txDetails.Hash(), err)
			continue
		}
...
func (c *BitcoindClient) filterTx(txDetails *btcutil.Tx,
	...
	*wtxmgr.TxRecord, error) {
	...
	if err != nil {
		log.Log.Errorf("Cannot create transaction record for relevant "+
			"tx: %v", err)
