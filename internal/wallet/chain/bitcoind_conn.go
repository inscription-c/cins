...
type BitcoindConn struct {
	...
	prunedBlockDispatcher *chain.PrunedBlockDispatcher
...
func NewBitcoindConn(cfg *BitcoindConfig) (*BitcoindConn, error) {
	...
	// node is pruned.
	var prunedBlockDispatcher *chain.PrunedBlockDispatcher
	if chainInfo.Pruned {
		prunedBlockDispatcher, err = chain.NewPrunedBlockDispatcher(
			&chain.PrunedBlockDispatcherConfig{
...
func (c *BitcoindConn) Start() error {
	...
	if c.prunedBlockDispatcher != nil {
		log.Log.Debug("Detected pruned bitcoind backend")
...
func (c *BitcoindConn) Stop() {
	...
	if err := c.events.Stop(); err != nil {
		log.Log.Errorf("error shutting down bitcoind events: %w", err)
	}
...
func (c *BitcoindConn) NewBitcoindClient() *BitcoindClient {
	return &BitcoindClient{
		...
		notificationQueue: queue.NewConcurrentQueue(20),
