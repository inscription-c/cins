...
func (b *bitcoindRPCPollingEvents) blockEventHandlerRPC(startHeight int32) {
	...
	height := startHeight
	log.Log.Infof("Started polling for new bitcoind blocks via RPC at "+
		"height %d", height)
	...
	for {
		...
		case <-ticker.C:
			...
			info, err := b.client.GetBlockChainInfo()
			if err != nil {
				log.Log.Errorf("Unable to retrieve best block: "+
					"%v", err)
				continue
			}
			...
			for i := height + 1; i <= info.Blocks; i++ {
				...
				if err != nil {
					log.Log.Errorf("Unable to retrieve "+
						"block hash: %v", err)
					continue
				}
...
func (b *bitcoindRPCPollingEvents) txEventHandlerRPC() {
	...
	log.Log.Info("Started polling mempool for new bitcoind transactions via RPC.")
