...
func newBitcoindZMQEvents(cfg *ZMQConfig, client *rpcclient.Client,
	...
	if cfg.PollingIntervalJitter < 0 {
		log.Log.Warnf("Jitter value(%v) must be positive, setting to 0",
	...
	if err != nil {
		...
		if err := zmqBlockConn.Close(); err != nil {
			log.Log.Errorf("could not close zmq block conn: %v", err)
		}
...
func (b *bitcoindZMQEvents) blockEventHandler() {
	...
	log.Log.Info("Started listening for bitcoind block notifications via ZMQ "+
	...
	for {
		...
		if err != nil {
			...
			if ok && netErr.Timeout() {
				log.Log.Trace("Re-establishing timed out ZMQ " +
					"block connection")
				continue
			}

			log.Log.Errorf("Unable to receive ZMQ %v message: %v",
		...
		case rawBlockZMQCommand:
			...
			if err := block.Deserialize(r); err != nil {
				log.Log.Errorf("Unable to deserialize block: %v",
					err)
				continue
			}
		...
		default:
			...
			log.Log.Warnf("Received unexpected event type from %v "+
...
func (b *bitcoindZMQEvents) txEventHandler() {
	...
	log.Log.Info("Started listening for bitcoind transaction notifications "+
	...
	for {
		...
		case rawTxZMQCommand:
			...
			if err := tx.Deserialize(r); err != nil {
				log.Log.Errorf("Unable to deserialize "+
					"transaction: %v", err)
				continue
			}
...
func (b *bitcoindZMQEvents) mempoolPoller() {
	...
	log.Log.Info("Started polling mempool to cache new transactions")

	// Create a ticker that fires randomly.
	rand.Seed(time.Now().UnixNano())
	ticker := chain.NewJitterTicker(
	...
	for {
		select {
		case <-ticker.C:
			log.Log.Tracef("Reconciling mempool spends with node " +
				"mempool...")
			...
			log.Log.Tracef("Reconciled mempool spends in %v",
				time.Since(now))
