...
func (c *cachedInputs) addInput(op wire.OutPoint, txid chainhash.Hash) {
	...
	if isReplacement {
		log.Log.Tracef("Input %s was spent in tx %s, now spent in %s",
			op, oldTxid, txid)
...
func (m *mempool) Shutdown() {
	log.Log.Debug("Local mempool shutting down...")
	defer log.Log.Debug("Local mempool shutdown complete")
...
func (m *mempool) add(tx *wire.MsgTx) {
	...
	if blockchain.IsCoinBaseTx(tx) {
		log.Log.Debugf("Skipping coinbase tx %v", tx.TxHash())
		return
	}
...
func (m *mempool) WaitForInit() {
	...
	case <-m.initFin:
	case <-m.quit:
		log.Log.Debugf("Mempool shutting down before init finished")
	}
}
...
func (m *mempool) LoadMempool() error {
	log.Log.Debugf("Loading mempool spends...")
	now := time.Now()

	// Fetch the latest mempool.
	txids, err := m.cfg.rawMempoolGetter()
	if err != nil {
		log.Log.Errorf("Unable to get raw mempool txs: %v", err)
		return err
	}
	...
	go func() {
		...
		if err != nil {
			log.Log.Errorf("LoadMempool got error: %v", err)
		}

		log.Log.Debugf("Loaded mempool spends in %v", time.Since(now))
		close(m.initFin)
	}()
...
func (m *mempool) UpdateMempoolTxes() []*wire.MsgTx {
	...
	for _, txHash := range txids {
		...
		if m.isShuttingDown() {
			log.Log.Info("UpdateMempoolTxes exited due to shutdown")
	...
	if err != nil {
		log.Log.Error("Batch getrawtransaction got %v", err)
...
func (m *mempool) getRawMempool() ([]*chainhash.Hash, error) {
	...
	if err != nil {
		log.Log.Errorf("Unable to send GetRawMempool: %v", err)
		return nil, err
	}
...
func (m *mempool) batchGetRawTxes(txids []*chainhash.Hash,
	...
	log.Log.Debugf("Batching GetRawTransaction in %v batches...",
		uint32(len(txids))/m.cfg.getRawTxBatchSize+1)
	defer log.Log.Debugf("Finished batch GetRawTransaction")
	...
	for i, txHash := range txids {
		...
		// If so, we'll exit early.
		if m.isShuttingDown() {
			log.Log.Info("LoadMempool exited due to shutdown")
			return nil, nil
		}
		...
		if uint32(len(respReceivers))%m.cfg.getRawTxBatchSize == 0 {
			log.Log.Debugf("Processing GetRawTransaction for batch "+
...
func getRawTxIgnoreErr(txid chainhash.Hash,
	...
	if strings.Contains(errStr, errExp) {
		log.Log.Debugf("unable to fetch transaction %s from mempool: %v",
			txid, err)

	} else {
		// Otherwise, unexpected error is found, we'll create an error
		// log.
		log.Log.Errorf("unable to fetch transaction %s from mempool: %v",
			txid, err)
	}
