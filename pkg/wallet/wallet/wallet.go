...
func (w *Wallet) recovery(chainClient chain.Interface,
	...
	currentNum := int32(16)
	heights := bestHeight - startHeight + 1
	if heights < currentNum {
		currentNum = heights
	}
	batchNum := int32(math.Ceil(float64(heights) / float64(currentNum)))
	blocks := make([]*waddrmgr.BlockStamp, 0)

	for i := int32(0); i < batchNum; i++ {
		currentBatch := i
		currentGetBlocks := make([]*waddrmgr.BlockStamp, currentNum)
		for j := int32(0); j < currentNum; j++ {
			...
			errWg.Go(func() error {
				...
				currentGetBlocks[idx] = &waddrmgr.BlockStamp{
					Hash:      *hash,
					Height:    height,
					Timestamp: header.Timestamp,
				}
		...
		currentHeight += currentNum

		for i := 0; i < len(currentGetBlocks); i++ {
			currentGetBlock := currentGetBlocks[i]
			if currentGetBlock == nil {
				break
			}
			...
			if currentGetBlock.Height >= birthdayBlock.Height {
				recoveryMgr.AddToBlockBatch(
					&currentGetBlock.Hash, currentGetBlock.Height, currentGetBlock.Timestamp,
				)
			}
			blocks = append(blocks, currentGetBlock)
		}
		...
		if len(recoveryBatch) >= recoveryBatchSize || currentHeight >= bestHeight {
			err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
				ns := tx.ReadWriteBucket(waddrmgrNamespaceKey)
